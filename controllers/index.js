...
router.post('/youtubei/search', (req, res) => {
  const { continuation, query, apiKey, clientVersion } = req.body || {};
  if (!continuation && !query) return res.status(400).json({ error: 'missing continuation or query' });
  const key = apiKey || 'AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8';
  const cv = clientVersion || '2.20240101.00.00';
  const payload = {
    context: { client: { clientName: 'WEB', clientVersion: cv, hl: 'en', gl: 'US' } }
  ...
  fetch(`https://www.youtube.com/youtubei/v1/search?key=${encodeURIComponent(key)}&prettyPrint=false`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Origin': 'https://www.youtube.com',
