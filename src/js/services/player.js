...
import YoutubeIframePlayer from './youtube_iframe_player';
...
class Player {
  constructor() {
    ...
    YoutubeIframePlayer.injectHandlers(this.playstateChange, this.songEnded);
  ...
  getPlayerFor(song, options) {
    ...
    if (song.isYoutube) {
      if (!song.inLibrary) {
        return new YoutubeIframePlayer(song, options);
      }
